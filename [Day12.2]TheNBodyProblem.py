...
#     moons = load()
#
#     states = run_simulation(moons, steps=100000000)
#
#     # with open("input/moons.csv", "w", newline="") as f:
#     #     writer = csv.writer(f)
#     #     for s in states:
#     #         writer.writerow(s)
#
...
#
#
# def run_simulation(moons: List[Moon], steps: int) -> Set[Tuple[int, ...]]:
#     states = set()
#     moon_states = {i: set() for i in range(len(moons))}
#     for step in range(steps):
#         curr_states = [tuple([p for p in m.pos] + [v for v in m.vel]) for m in moons]
#         for i, curr_state in enumerate(curr_states):
#             if curr_state in moon_states[i]:
#                 print("Moon %d repeated state on step %d" % (i + 1, step))
#             moon_states[i].add(curr_state)
#         states.add(tuple(x for i in range(len(moons)) for x in curr_states[i]))
